    ...
    <header className="fixed w-full z-20 backdrop-blur-xl bg-white/50">
        ...
        <Image 
            ...
            <div className="relative flex gap-10 ">
                <FiSearch size={24}/>
                <Button variant="ghost" className="relative cursor-pointer " onClick={() => setIsCartPopupOpen(!isCartPopupOpen)}>
                    <FiShoppingBag size={24}/>
                    {
                      items.length ?  (
                        <div className=" rounded-full w-3.5 h-3.5 absolute -top-1 -right-1 text-[10px] text-white text-center">
                      {items.length}
                    </div>
                      ):(
                        <></>
